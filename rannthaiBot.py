...
class FetchProductData:
    ...
    def find_data(self, description, origin,credentials):
        ...
        while(check):
            try:
                ...
                base_url = 'https://www.rannthai.com/en/search.php?a=' #rannthai search
                params = credentials.get_proxycrawl_credentails(url = base_url + description + '&lg=en&Submit2=search')
                ...
                check = True
                pass
        if len(web_soup.findAll('div',{'class':'bxmodul'})) != 0: #class
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('div', {'class':'modul'}) #class
        price_list = web_soup.findAll('p',{'class':'pricing'})
            ...
            for Data_from_websites in list_of_products:
                ...
                if price_from_website is not None and price_from_website != '':
                    try:
                        ...
                        product_description = Data_from_websites.find('p',{'class':'ptitle'}).text
                        #P1= price_from_website
                        price_from_website = price_from_website.replace("US", "")
                    ...
                    product_description = product_description.lower()
                    product_origin = Data_from_websites.find('p',{'class':'country'}).text
                    description = description.lower()
