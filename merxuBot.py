...
class FetchProductData:
    ...
    def find_data(self, description, origin,credentials):
        ...
        while(check):
            try:
                ...
                base_url = 'https://merxu.com/en/search/?q=' #merxu search
        ...
        if len(web_soup.findAll('a')) != 0: #class #!!
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('a', {'class':'result-item-list'}) #class
        price_list = web_soup.findAll('span',{'class':'price_whhb3'}) #class
        index=0
        try:
            for Data_from_websites in list_of_products:
                ...
                if price_from_website is not None and price_from_website != '':
                    try:
                        link = Data_from_websites.get('href')
                        product_description = Data_from_websites.find(["h3"]).text
                        ...
                        prices=str(P1).replace("EUR",'').split('/')
                        min_order_weight = 1
                    ...
                    product_description = product_description.lower()
                    #product_origin = Data_from_websites.find('span',{'class':'price'}).text
                    #product_origin = product_origin['title']
                    product_origin = "EUROPE"
                    description = description.lower()
