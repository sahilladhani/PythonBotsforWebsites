...
class FetchProductData:
    ...
    def find_data(self, description, origin,credentials):
        ...
        while(check):
            try:
                ...
                base_url = 'https://www.ecplaza.net/' #ecplaza search
                params = credentials.get_proxycrawl_credentails(url = base_url + description + '--product')
                ...
                check = True
                pass
        if len(web_soup.findAll('div',{'class':'row'})) != 0: #class
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('div', {'class':'product-box'}) #class
        price_list = web_soup.findAll('span',{'class':'payment-info__price'})
        ...
        try:
            for Data_from_websites in list_of_products:
                ...
                if price_from_website is not None and price_from_website != '':
                    try:
                        product_description = Data_from_websites.find('h4').text
                        link = Data_from_websites.find('h4').find('a').get('href')
                    ...
                    product_origin =  Data_from_websites.find('span',{'class':'location'}).text
                    product_origin = product_origin.replace("\n                    ", "")
                    product_origin = product_origin.replace("                ", "")
                    description = description.lower()
