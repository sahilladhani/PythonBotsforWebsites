...
class FetchProductData:
    ...
    def find_data(self, description, origin,credentials):
        ...
        while(check):
            try:
                ...
                base_url = 'http://gsearch.gmarket.co.kr/Listview/Search?keyword=' #gmarket search
                ...
                check = True
                pass
        if len(web_soup.findAll('tbody',{'id':'srplist'})) != 0: #class
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('ul',{'class':'produt_info'}) #class
        price_list = web_soup.findAll('li',{'class':'discount_price'})
            ...
            for Data_from_websites in list_of_products:
                ...
                if price_from_website is not None and price_from_website != '':
                    try:
                        ...
                        product_description = Data_from_websites.find('li',{'class':'item_name'}).text
                        P1= price_from_website
                        currency ="KRW"
                        unit_of_measurement = None
                        prices=str(P1).replace("KRW",'').split('/')
                        min_order_weight = 1
