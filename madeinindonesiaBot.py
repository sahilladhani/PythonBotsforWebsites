...
class FetchProductData:
    ...
    def find_data(self, description, origin,credentials):
        ...
        while(check):
            try:
                ...
                base_url = 'https://madeinindonesia.com/search/?subcats=Y&pcode_from_q=Y&pshort=Y&pfull=Y&pname=Y&pkeywords=Y&search_performed=Y&sort_by=position&sort_order=asc&match=any&minimum_chars=3&q=' #madeinindonesia search
                params = credentials.get_proxycrawl_credentails(url = base_url + description + '&security_hash=cb7bb58bb7d89b883836a45855b9b102')
                ...
                check = True
                pass
        if len(web_soup.findAll('div',{'class':'grid-list'})) != 0: #class
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('div', {'class':'ty-column5'}) #class 
        
        price_list = web_soup.findAll('div',{'class':'ty-grid-list__price'})
            ...
            for Data_from_websites in list_of_products:
                ...
                if price_from_website is not None and price_from_website != '':
                    try:
                        ...
                        product_description = Data_from_websites.find('div',{'class':'ty-grid-list__item-name'}).text
                        price_from_website = price_from_website.replace("\n", "")
                        price_from_website = price_from_website.replace("                        ", "")
                        price_from_website = price_from_website.replace("/ Unit                     ", "")
