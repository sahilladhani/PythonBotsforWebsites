...
class FetchProductData:
    ...
    def find_data(self, description, origin,credentials):
        ...
        while(check):
            try:
                ...
                base_url = 'https://shoptheglobe.co/?s=' #shoptheglobe search
                params = credentials.get_proxycrawl_credentails(url = base_url + description + '&id=8744&post_type=product')
                ...
                check = True
                pass
        if len(web_soup.findAll('div',{'id':'woo-products-wrap'})) != 0: #class
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('li', {'class':'product'}) #class
        price_list = web_soup.findAll('del')
