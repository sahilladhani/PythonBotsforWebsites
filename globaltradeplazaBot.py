...
class FetchProductData:
    ...
    def find_data(self, description, origin,credentials):
        ...
        while(check):
            try:
                ...
                base_url = 'https://globaltradeplaza.com/products/?search-keyword=' #globaltradeplaza search
                params = credentials.get_proxycrawl_credentails(url = base_url + description + '&type=1')
                ...
                check = True
                pass
        if len(web_soup.findAll('div',{'id':'showProducts'})) != 0: #class
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('div', {'class':'cards'}) #class
        price_list = web_soup.findAll('ul',{'class':'attributes'})
            ...
            for Data_from_websites in list_of_products:
                ...
                if price_from_website is not None and price_from_website != '':
                    try:
                        ...
                        product_description = Data_from_websites.find('h5').text
                        price_from_website = price_from_website.replace("Place of Origin - nullBusiness Type - Payment - Price - ", "")
