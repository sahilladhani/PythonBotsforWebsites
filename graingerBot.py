...
class FetchProductData:
    ...
    def find_data(self, description, origin, credentials):
        ...
        while(check): #proxy check
            try:

                base_url = 'https://www.grainger.com/search?searchQuery=' #website URL
                params = credentials.get_proxycrawl_credentails(url = base_url + description +'&searchBar=true')
        ...
        if len(web_soup.findAll('div', {'class':'pane'})) != 0: #main product class
            data = self.fetch_data(web_soup, description, origin)
            print(data)
        else:
           data = []
           print("No Products Found") #testing

        # return_dict['grainger'] = data
        # return return_dict
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('div',{'class':'search-results__product-column-right'}) #product main class here
        listone = (web_soup.findAll('span', {'class':'pricing__price'}))
        listtwo = (web_soup.findAll('p', {'class':'search-results__product-buy-box-discontinued'}))
        price_list = listone + listtwo 

        #price_list = price_list['title']
        index=0
        try:
            for Data_from_websites in list_of_products:
                price_from_website = price_list[index].text
                if price_from_website is None:
                    price_from_website = "Price is Discontinued"
                else:
                    price_from_website = price_list[index].text
                ...
                if price_from_website is not None and price_from_website != '':
                    try:
                        ...
                        brand_title = Data_from_websites.find('div',{'class':'search-results__product-brand-container'}).text
                        product_title = Data_from_websites.find('div',{'class':'search-results__product-description'}).text
                        product_description = brand_title + " " + product_title 
                        #_____
                        P1= price_from_website
                        prices = P1
                        prices = prices.replace("                                                                            ", "")     
                        currency ="USD" #currency here
                        unit_of_measurement = None
                        #prices=str(P1).replace("USD",'').split('/') #currency
                        ...
                        print(ex)
                        print("line 201 EXCEPTION") #exception
                            # prices = [float(p) for p in re.findall(r'-?\d+\.?\d*', list_of_prices)]

                    product_description = product_description.lower()
                    #product_origin = Data_from_websites.find('span',{'class':'c2i43-'}) #product origin class here
                    #product_origin = product_origin['title']
                    product_origin = "USA"
                    description = description.lower()
                    #reCheck = re.search(r'(\D*)?(\d+)\s*/?(\w+)$', product_description) #x
...
#grainger.com
#table view products not searching
#end