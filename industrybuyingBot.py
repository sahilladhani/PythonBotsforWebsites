...
class FetchProductData:
    ...
    def find_data(self, description, origin,credentials):
        ...
        while(check):
            try:
                ...
                base_url = 'https://www.industrybuying.com/search/?q=' #industrybuying search
                params = credentials.get_proxycrawl_credentails(url = base_url + description + '&cat=')
                ...
                check = True
                pass
        if len(web_soup.findAll('div',{'class':'AH_ProductListInner'})) != 0: #class
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('div', {'class':'AH_ProductView'}) #class
        price_list = web_soup.findAll('span',{'class':'rs'})
            ...
            for Data_from_websites in list_of_products:
                ...
                if price_from_website is not None and price_from_website != '':
                    try:
                        ...
                        product_description = Data_from_websites.find('a',{'class':'prFeatureName'}).text
