...
class FetchProductData:
    ...
    def find_data(self, description, origin,credentials):
        ...
        while(check):
            try:
                ...
                base_url = 'https://www.mallory.com/searchPage.action?keyWord=' #mallory search
                params = credentials.get_proxycrawl_credentails(url = base_url + description + '&overRideCatId=N')
                ...
                check = True
                pass
        if len(web_soup.findAll('div',{'class':'listGridContainer'})) != 0: #class
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('div', {'data-getchangemode':'getchangemode'}) #class
        price_list = web_soup.findAll('li',{'class':'cimm_priceWrap'})
        ...
        try:
            for Data_from_websites in list_of_products:
                ...
                price_from_website = price_from_website.replace("/EA", "")
                ...
                if price_from_website is not None and price_from_website != '':
                    try:
                        product_description = Data_from_websites.find('h3',{'class':'cimm_productTitle'}).text
                        product_description = product_description.replace("\n                            ", "")
                        product_description = product_description.replace("\n                          ", "")

                        link = Data_from_websites.find('h3',{'class':'cimm_productTitle'}).find('a').get('href')
