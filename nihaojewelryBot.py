...
class FetchProductData:
    ...
    def find_data(self, description, origin,credentials):
        ...
        while(check):
            try:
                ...
                base_url = 'https://www.nihaojewelry.com/productSearch/result?category=0&query=' #nihaojewelry search
                ...
                check = True
                pass
        if len(web_soup.findAll('div',{'class':'goods-wrap'})) != 0: #class
    ...
    def fetch_data(self, web_soup, description, origin):
        ...
        list_of_products = web_soup.findAll('div',{'class':'j_goods_item'}) #class
        price_list = web_soup.findAll('span',{'class':'original-price'})
            ...
            for Data_from_websites in list_of_products:
                ...
                if price_from_website is not None and price_from_website != '':
                    try:
                        ...
                        product_description = Data_from_websites.find('p',{'class':'remark'}).text
                        product_description = product_description.replace("                                    ", "")
                        product_description = product_description.replace("                         ", "")
                        P1= price_from_website
                        currency ="USD"
                        unit_of_measurement = None
                        prices=str(P1).replace("US",'').split('/')
                        min_order_weight = 1
                    ...
                    product_descripiton = product_description.lower()
                    product_origin = None
                    description = description.lower()
